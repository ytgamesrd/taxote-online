    ...
    try {
      ...
      if (path === "/api/admin/login" && method === "POST") {
        ...
        if (body.username === "TAXOTEadmin1995" && body.password === "123Taxote123@1995") {
          ...
          await db.prepare("INSERT INTO admin_sessions(token_hash,expires_at,created_at) VALUES(?,?,?)")
            .bind(await sha256(token), expiresAt, nowIso()).run();
      ...
      if (path === "/api/admin/logout" && method === "POST") {
        ...
        if (token) await db.prepare("DELETE FROM admin_sessions WHERE token_hash=?").bind(await sha256(token)).run();
        return json({ ok: true }, 200, {
      ...
      const adminCookie = parseCookies(request).taxote_admin_session;
      let isAdmin = false;
      if (adminCookie) {
        const row = await db.prepare("SELECT expires_at FROM admin_sessions WHERE token_hash=?").bind(await sha256(adminCookie)).first();
        isAdmin = Boolean(row && new Date(row.expires_at).getTime() > Date.now());
        if (row && !isAdmin) await db.prepare("DELETE FROM admin_sessions WHERE token_hash=?").bind(await sha256(adminCookie)).run();
      }
      ...
      const isProtectedApi = path.startsWith("/api/admin/") || path.startsWith("/api/dispatch/");

      if (isProtectedApi && !isAdmin) throw new HttpError(401, "No autorizado");
