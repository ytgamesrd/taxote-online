      ...
      if (path === "/api/admin/login" && method === "POST") {
        ...
        if (body.username === "TAXOTEadmin1995" && body.password === "123Taxote123@1995") {
          ...
          return json({ ok: true, token }, 200, {
            "Set-Cookie": `taxote_admin_session=${token}; Path=/; Secure; SameSite=Lax; HttpOnly; Domain=${url.hostname}`,
            ...headers
          });
        }
...
async function handleApi(request, env, url) {
  ...
  if (path === "/api/driver/register" && method === "POST") {
    const body = await bodyJson(request);
    const p = phone(body.phone), salt = crypto.randomUUID().replaceAll("-", ""), hash = await passwordHash(body.password, salt), did = id("DRV"), stamp = nowIso();
    const result = await db.prepare(`INSERT INTO drivers(public_id,first_name,last_name,email,phone,password_hash,password_salt,cedula,vehicle_type,vehicle_brand,vehicle_model,vehicle_color,vehicle_plate,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,'pending',?,?)`).bind(did, clean(body.firstName), clean(body.lastName), clean(body.email).toLowerCase(), p, hash, salt, clean(body.cedula), clean(body.vehicleType), clean(body.vehicleBrand), clean(body.vehicleModel), clean(body.vehicleColor), clean(body.vehiclePlate).toUpperCase(), stamp, stamp).run();
  ...
  if (path === "/api/driver/login" && method === "POST") {
    ...
    return json({ ok: true, driver: driverView(driver) }, 200, {
      "Set-Cookie": `taxote_driver_session=${token}; Path=/; Secure; SameSite=Lax; HttpOnly; Domain=${url.hostname}`,
      ...headers
    });
  }
  ...
  let match = path.match(/^\/api\/admin\/drivers\/([^/]+)$/);
  if (match && method === "GET") {
    const driver = await db.prepare("SELECT * FROM drivers WHERE public_id=?").bind(decodeURIComponent(match[1])).first();
    if (!driver) throw new HttpError(404, "Conductor no encontrado.");
    return json({ driver: driverView(driver, true) }, 200, headers);
  }

  match = path.match(/^\/api\/admin\/drivers\/([^/]+)\/document\/([^/]+)$/);
  if (match && method === "GET") {
    const driver = await db.prepare("SELECT id FROM drivers WHERE public_id=?").bind(decodeURIComponent(match[1])).first();
    const doc = await db.prepare("SELECT data_url FROM driver_documents WHERE driver_id=? AND kind=?").bind(driver.id, decodeURIComponent(match[2])).first();
    if (!doc) throw new HttpError(404, "Documento no encontrado.");
    const parts = doc.data_url.split(",");
    const bin = atob(parts[1]), arr = new Uint8Array(bin.length);
    for (let i = 0; i < bin.length; i++) arr[i] = bin.charCodeAt(i);
    return new Response(arr, { headers: { "Content-Type": parts[0].match(/:(.*?);/)[1], ...headers } });
  }
  ...
  if (path === "/api/admin/driver-locations" && method === "GET") {
    ...
    return json(results.map(r => ({ id: r.public_id, name: `${r.first_name} ${r.last_name}`.trim(), phone: r.phone, location: { lat: Number(r.current_lat), lon: Number(r.current_lon), bearing: Number(r.current_bearing || 0) }, vehiclePlate: r.vehicle_plate, connectionState: 'available' })), 200, headers);
  }
